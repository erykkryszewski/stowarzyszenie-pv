        ...
        <div class="theme-blog__wrapper">
            ...
            <div class="row">
                ...
                <div class="<?php echo esc_html($column_classes); ?> theme-blog__column">
                    <?php $terms = get_the_terms(get_the_ID(), 'category') ?: []; $mods = array_map(fn($t) => 'theme-blog__item--' . sanitize_html_class($t->slug), $terms); $theme_blog_item = 'theme-blog__item' . (!empty($mods) ? ' ' . implode(' ', $mods) : ''); $is_video = false; if (!empty($terms)) { foreach ($terms as $term) { if ($term->slug === 'video') { $is_video = true; break; } } } ?>

                    <div class="<?php echo esc_attr($theme_blog_item); ?>">
                        <div
                            class="theme-blog__image <?php if ($posts_number == 2) { echo 'theme-blog__image--bigger'; } ?> <?php if ($posts_number == 4) { echo 'theme-blog__image--smaller'; } ?> <?php if ($is_video) { echo 'theme-blog__image--video'; } ?>"
                        >
                        ...
                        <div class="theme-blog__content">
                            ...
                            <a href="<?php the_permalink(); ?>" class="theme-blog__button">
                                <?php if ($is_video): ?> <?php _e('Zobacz video', 'ercodingtheme'); ?> <?php else: ?> <?php _e('Czytaj więcej', 'ercodingtheme'); ?> <?php endif; ?>
                            </a>
                        </div>
                    </div>
                </div>

...
<html lang="<?php bloginfo('language'); ?>">
    <head>
        ...
        <?php echo !empty($header_code) ? $header_code : ''; ?> <?php wp_head(); ?>
    </head>
                ...
                <div class="container">
                    <div class="top-bar__wrapper">
                        ...
                        <div class="top-bar__content top-bar__content--right">
                            <a href="http://panel.stowarzyszeniepv.pl/" target="_blank" class="top-bar__panel">Strefa członkowska</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="container">
                <nav class="nav <?php if (!is_front_page()) { echo 'nav--subpage'; } ?>">
                    ...
                    <div class="nav__content <?php if (!is_front_page()) { echo 'nav__content--subpage'; } ?>">
                        <?php $menu_class = is_front_page() ? 'nav__menu' : 'nav__menu nav__menu--subpage'; echo wp_nav_menu([ 'theme_location' => 'Navigation', 'container' => 'ul', 'menu_class' => $menu_class, ]); ?> <?php if (!empty($header_button) || !empty($header_second_button)): ?>
                        <div class="nav__buttons">
                            <?php if (!empty($header_button)): ?>
                            <a
                                href="<?php echo esc_url($header_button['url']); ?>"
                                target="<?php echo !empty($header_button['target']) ? esc_attr($header_button['target']) : '_self'; ?>"
                                class="button nav__button nav__button--first <?php if (!is_front_page()) { echo 'nav__button--subpage'; } ?>"
                            >
                                <?php echo esc_html($header_button['title']); ?>
                            </a>
                            <?php endif; ?> <?php if (!empty($header_second_button)): ?>
                            <a
                                href="<?php echo esc_url($header_second_button['url']); ?>"
                                target="<?php echo !empty($header_second_button['target']) ? esc_attr($header_second_button['target']) : '_self'; ?>"
                                class="button nav__button nav__button--second <?php if (!is_front_page()) { echo 'nav__button--subpage'; } ?>"
                            >
